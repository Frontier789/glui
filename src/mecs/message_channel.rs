...
impl MessageChannel {
    ...
    where
        M: Message,
    {
        self.send_annotated((MessageTarget::Broadcast, message).into())
    }

    pub fn send_annotated(&self, msg: AnnotatedMessage) -> Result<(), AnnotatedMessage> {
        match &self.implementation {
            ChannelImpl::HandRolled(sender) => match sender.send(msg) {
                Ok(_) => Ok(()),
                Err(SendError(msg)) => Err(msg),
            },
            ChannelImpl::Glutin(proxy) => match proxy.send_event(msg) {
                Ok(_) => Ok(()),
