...
pub trait GlUniform {
    fn set_uniform_impl(val: Self, loc: GLint, map: &mut HashMap<GLint, (u32, u32)>);
}

impl GlUniform for f32 {
    fn set_uniform_impl(val: Self, loc: GLint, _map: &mut HashMap<GLint, (u32, u32)>) {
        unsafe {
...
impl GlUniform for &Texture {
    fn set_uniform_impl(val: Self, loc: GLint, map: &mut HashMap<GLint, (u32, u32)>) {
        ...
        if map.contains_key(&loc) {
            ...
            unsafe {
                gl::Uniform1i(loc, slot as GLint);
            }
            map.insert(loc, (slot, id));
        }
    }
