...
use gui::{GuiCallback, PostBox, Widget, WidgetBuilderCache, WidgetList, GuiBuilder};
...
impl WidgetParser {
	...
	pub fn add_callback<T>(cb: StoredCallback) -> GuiCallback<T> where T: Widget {
		let mut id: usize = 0;
		WIDGETPARSER_INSTANCE.with(|widget_parser| {
			let mut widget_parser = widget_parser.borrow_mut();
    ...
    pub(crate) fn remove_callback<T>(cb: &GuiCallback<T>) where T: Widget {
